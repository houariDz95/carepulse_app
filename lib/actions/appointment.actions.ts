...
export const updateAppointment = async ({appointmentId, userId, appointment, type} : UpdateAppointmentParams) => {
  try {
    const updateAppoinment = await databases.updateDocument(
      DATABASE_ID,
      APPOINTMENT_COLLECTION_ID,
      appointmentId,
      appointment
    ...
    if(!updateAppoinment){
      ...
      const smsMessage = `
        ...
        : `We regret to inform you that your appointment has been canncelled. Reason ${appointment.canncellationReason!}` }
      `;

      await sendSmsNotification(userId, smsMessage)
      revalidatePath('/admin');
      return parseStringify(updateAppoinment)
    }
  } catch (error) {
    console.log(error)
  }
